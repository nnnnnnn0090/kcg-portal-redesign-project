...
function NavButton({
  ...
  return (
    <button
      className={cn(
        'community-nav-button tw-flex tw-min-h-[38px] tw-items-center tw-gap-3 tw-rounded-lg tw-border-0 tw-bg-transparent tw-px-3 tw-text-sm tw-font-semibold tw-text-community-muted tw-cursor-pointer tw-transition-colors hover:tw-bg-community-accent-bg hover:tw-text-community-accent-light [&.is-active]:tw-bg-community-accent-bg [&.is-active]:tw-text-community-accent-light max-[960px]:tw-justify-center max-[960px]:tw-px-0 max-[960px]:[&>span]:tw-hidden [&>svg]:tw-h-[18px] [&>svg]:tw-w-[18px] [&>svg]:tw-flex-none [&>svg]:tw-fill-none [&>svg]:tw-stroke-current [&>svg]:tw-stroke-[1.9]',
        active && 'is-active',
      )}
      onClick={onClick}
    >
      ...
      {badge ? (
        <b
          className={
            'community-nav-badge tw-ml-auto tw-min-w-5 tw-rounded-full tw-bg-community-danger tw-px-1.5 tw-text-center tw-text-xs tw-leading-[18px] tw-text-white max-[960px]:tw-absolute max-[960px]:tw-ml-6 max-[960px]:tw-mb-6'
          }
        >
          {badge > 99 ? '99+' : badge}
        </b>
