...
export function AuthDialog(props: ModalLayerProps & { mode: 'login' | 'register' }) {
  ...
  return (
    <div
      role="form"
      className={cn(
        'community-dialog community-auth tw-relative tw-grid tw-max-h-[min(92vh,820px)] tw-w-full tw-overflow-hidden tw-rounded-2xl tw-border-2 tw-border-community-border tw-bg-community-bg tw-shadow-[0_0_0_1px_color-mix(in_srgb,var(--p-accent)_18%,transparent),0_22px_80px_color-mix(in_srgb,#000_34%,transparent)] tw-animate-community-dialog-in tw-transition-[max-width,grid-template-columns] tw-duration-300 tw-ease-[cubic-bezier(.2,.8,.2,1)] max-[520px]:tw-max-h-[calc(100vh-16px)] max-[520px]:tw-rounded-xl max-[720px]:tw-block',
        registering
          ? 'tw-max-w-[760px] tw-grid-cols-[220px_minmax(0,1fr)] max-[720px]:tw-max-w-[480px]'
          : 'tw-max-w-[420px] tw-grid-cols-[0fr_minmax(0,1fr)] max-[720px]:tw-max-w-[420px]',
      )}
      autoComplete="off"
      data-1p-ignore
      data-lpignore="true"
    ...
    >
      <button
        className={
          'tw-absolute tw-right-3 tw-top-3 tw-z-[5] tw-grid tw-h-10 tw-w-10 tw-place-items-center tw-rounded-lg tw-border-0 tw-bg-transparent tw-p-0 tw-text-community-muted tw-cursor-pointer tw-transition-[background-color,color,transform] tw-duration-180 hover:tw-bg-community-bg3 hover:tw-text-community-text hover:tw-rotate-90 active:tw-scale-95 focus-visible:tw-bg-community-bg3 focus-visible:tw-text-community-text focus-visible:tw-outline-none focus-visible:tw-ring-2 focus-visible:tw-ring-community-accent-bg [&_svg]:tw-h-[18px] [&_svg]:tw-w-[18px] [&_svg]:tw-fill-none [&_svg]:tw-stroke-current'
        }
        type="button"
        onClick={close}
      ...
      <aside
        ...
        className={cn(
          'tw-min-w-0 tw-overflow-hidden tw-border-r tw-border-community-border tw-bg-community-bg2 tw-transition-[opacity,padding] tw-duration-300 tw-ease-[cubic-bezier(.2,.8,.2,1)] max-[720px]:tw-hidden',
          registering
            ? 'tw-flex tw-flex-col tw-gap-7 tw-p-6 tw-opacity-100'
            : 'tw-pointer-events-none tw-p-0 tw-opacity-0',
        )}
      >
        <div className="tw-flex tw-min-w-[172px] tw-items-center tw-gap-3 [&_img]:tw-h-10 [&_img]:tw-w-10 [&_img]:tw-object-contain [&_strong]:tw-text-sm [&_strong]:tw-text-community-bright">
          <img src={browser.runtime.getURL('/community/activity-icon.png')} alt="" />
          <strong>{ja ? 'みんなの活動' : 'Campus Community'}</strong>
        </div>
        <div className="tw-grid tw-min-w-[172px] tw-gap-3 [&_small]:tw-text-[11px] [&_small]:tw-font-bold [&_small]:tw-text-community-accent-light [&_h2]:tw-m-0 [&_h2]:tw-text-xl [&_h2]:tw-leading-snug [&_h2]:tw-text-community-bright [&_p]:tw-m-0 [&_p]:tw-text-[13px] [&_p]:tw-leading-relaxed [&_p]:tw-text-community-muted">
          <small>CAMPUS COMMUNITY</small>
          <h2>{ja ? 'キャンパスの日常を、もっと近くに。' : 'Campus life, closer together.'}</h2>
          <p>
            {ja
              ? '気になる投稿を見つけたり、自分のひとことも残せます。'
              : 'Find posts that catch your eye, and leave a note of your own.'}
          </p>
        </div>
      </aside>
      ...
      >
        <div
          key={mode}
          className="tw-animate-community-auth-switch"
        >
          <div
            className={cn(
